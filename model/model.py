...
def get_questions(data):
    ...
    db=[x for x in db_data if "skill" in x or "competency" in x]
    db_skills={str(x["_id"]["$oid"]):" ".join(w for w in word_tokenize(x["skill"].lower() if x["skill"] else x["competency"].lower()) if w not in stop_words) for x in db if x["skill"] or x["competency"]}
    ...
    for i in data["skills"]:
        data_skill=[" ".join(w for w in word_tokenize(i.lower()) if w not in stop_words)]
